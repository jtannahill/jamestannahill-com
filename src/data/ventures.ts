...
export const ventures: Venture[] = [
  {
    ...
    page: {
      ...
      metaTitle: 'Plocamium Holdings — Patient Capital, Industry & Healthcare',
  ...
  },
  {
    ...
    page: {
      ...
      metaTitle: 'PROSEC Defense Group — Readiness Before the Crisis',
  ...
  },
  {
    ...
    page: {
      ...
      metaTitle: 'RDLB — Brand Architecture for Investors & Operators',
  ...
  },
  {
    ...
    page: {
      ...
      metaTitle: '1ness Strategies — Growth Systems for Regulated Industries',
  ...
  },
  {
    ...
    page: {
      ...
      metaTitle: 'NewYorkLab — Urban Intelligence from Sensor Data',
  ...
  },
  {
    ...
    page: {
      ...
      metaTitle: 'Nargusta — A Sailors\' Hospitality Network',
  ...
  },
  {
    name: 'SigScan',
    slug: 'sigscan',
    description: 'A signal radar for the iPhone. Every Bluetooth device around you appears as a bubble in augmented reality, named against the complete industry registries, ranged, and flagged when it should not be there. NFC tags, WiFi, and HomeKit accessories are read the same way, and nothing leaves the phone.',
    ...
    page: {
      ...
      body: [
        ...
        'Naming the thing turned out to be harder than finding it. Most scanners carry a shortlist of manufacturers and label the rest Unknown, and the ones that do better tend to report the radio rather than the product, so a doorbell built on an Espressif module reads as an Espressif device. SigScan ships the complete industry registries instead, treats the brand and the chip as the separate facts they are, and where a manufacturer publishes its advertising format it reads the product code out of the packet and names the model. Sensors that broadcast a temperature or a battery level are read as the packets pass, without connecting to anything. What kind of device it is comes from the packet too, and a brand that makes phones, televisions and earbuds under one identifier is never guessed at, so a television is not reported as a phone.',
        'What the app does not do is the other half. There are no accounts, no advertising, no analytics, and no upload: scans and locations are held on the phone in SwiftData and go nowhere. The one exception is the assistant, which sends a typed question and a short summary of device names to be answered, carrying no location and no identifier. An instrument for seeing who is listening should not itself be listening. What leaves the phone does so only when you send it: any device can be shared to Messages or AirDrop as plain text, and a home screen widget shows what the last scan found without opening the app.',
      ],
      links: [
      ...
      technical: {
        body: [
          ...
          'LiDAR does the rest. A mesh raycast clamps each bubble to the real surface in front of it, occlusion hides bubbles behind real walls, and a bubble whose LiDAR surface sits closer than its RSSI estimate is tinted to say so, which is what a device behind a wall looks like.',
          'Identity is a data problem and is solved as one. Nothing in the app names a company: a build step pulls the assigned-numbers registries, cross-checks them against an independent mirror, resolves ownership and restricted-vendor listings, and packs 22,888 entities and 44,975 identifiers into a single file the phone memory-maps and binary-searches. An advertisement is decomposed into signals, each signal is looked up in that one index, and the claims that come back are folded into an answer that keeps both its evidence and its runners-up. Decoders read bytes and emit further signals; they never decide who made anything, which is why adding a vendor is a row in a table rather than a branch in a function.',
        ],
        spec: [
          ...
          { label: 'Identity', value: '22,888 manufacturers, 44,975 identifiers, generated' },
          { label: 'Decoders', value: 'SwitchBot, Xiaomi MiBeacon, BTHome, Apple' },
          { label: 'Storage', value: 'SwiftData on device, GPS-stamped, CSV export' },
          { label: 'Assistant', value: 'Claude via a Cloudflare Worker proxy' },
          { label: 'Surfaces', value: 'Home screen widget, share sheet, custom tab bar' },
        ],
      },
      app: {
      ...
      metaTitle: 'SigScan — iPhone Signal Radar in Augmented Reality',
  ...
  },
  {
    ...
    page: {
      ...
      metaTitle: 'gOOOvy — Out-of-Office Replies for Google Voice',
  ...
  },
  {
    ...
    page: {
      ...
      technical: {
        body: [
          'The input is measurement, not mood: hourly Open-Meteo observations for the day\'s locations, and Sentinel-2 scenes sampled for colour, so a palette comes off actual ground and cloud rather than a designer\'s swatch. Nine of the seventeen artists render through their own weights, FLUX LoRAs trained on curated sets of a single artist\'s canvases, as few as fourteen where copyright limits what can be sourced.',
          'A vision model then scores every finished piece for fidelity to that artist, which is what drives the work: the artist rendering worst is the one who gets trained next.',
        ],
        spec: [
          { label: 'Data', value: 'Open-Meteo hourly, Sentinel-2 via Copernicus' },
          { label: 'Renderer', value: 'FLUX 1.1 Pro, with per-artist LoRAs' },
          { label: 'Training', value: '9 LoRAs, 14 to 66 canvases each' },
          { label: 'Evaluation', value: 'Vision model scores artist fidelity, 1 to 10' },
          { label: 'Pipeline', value: 'Step Functions and Python Lambdas on AWS, daily' },
        ],
      },
      metaTitle: 'Art. — Daily Weather Art from Atmospheric Data',
